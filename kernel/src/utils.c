...
//	if(connect(socket_servidor,servinfo->ai_addr,servinfo->ai_addrlen) == -1){
//		assert(!"no se pudo conectar al server");
//	}
...
void handshake(int conexion,char* nombre)
{
    ...
    if(i == 1){
        log_info(logger,"handshake realizado con %s",nombre);
    } else{ 
